...
(function($, window, document, undefined) {
    ...
    MaterializePagination.prototype = {
        ...
        createPaginationBase: function(requestedPage) {
            ...
            var $prevChevron = this.util.createChevron('prev', this.config.firstPage);
            var $nextChevron = this.util.createChevron('next', this.config.lastPage);
        ...
        util: {
            createPage: function(pageData) {
                ...
                search_params.set('page', pageData);
                ...
                if (pageData == "prev" || pageData == "next"){
                    $ali = $('<li>')
                        .html('<a href='+ new_url +'>' + pageData + '</a>')
                        .addClass('waves-effect')
                        .attr('data-page', pageData);
                }
            ...
            createChevron: function(type, limit) {
                var direction = type === 'next' ? 'right' : 'left';

                var urlParams = new URLSearchParams(window.location.search);
                var pageData = parseInt(urlParams.get('page'));
                ...
                if (type == "prev"){
                    search_params.set('page', (pageData-1).toString());
                }
                else if (type == "next"){
                    search_params.set('page', (pageData+1).toString());
                }
                ...
                if (pageData == limit || pageData == limit || isNaN(pageData)){
                    $a_chevron = $icon;
                    console.log('TRUE')
                }
                console.log($a_chevron);
